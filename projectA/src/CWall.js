...
function CWall(type) {
    ...
    if (this.wallType == WTYPE_VBOX_IN) {
        ...
        this.y = tonado_y;
        this.z = tonado_z;
    } else if (this.wallType == WTYPE_DEAD) {
        this.x = boid_x;
        this.y = boid_y;
        this.z = boid_z;
    } else if (this.wallType == WTYPE_VBALL_IN) {
        this.x = flame_x;
        this.y = flame_y;
        this.z = flame_z + 2.0;
    } else if (this.wallType == WTYPE_GROUND) {
        this.x = spring_x;
        this.y = spring_y;
        this.z = spring_z;
...
CWall.prototype.byBox = function(s, s0) {
    if (s.val[PART_XPOS] < this.xmin && s.val[PART_XVEL] < 0.0) {
        ...
        if (s.val[PART_XVEL] < 0.0)
            s.val[PART_XVEL] *= -g_rest;
            // bounce on left wall.
        }
    ...
    if (s.val[PART_YPOS] < this.ymin)
        s.val[PART_YPOS] = this.ymin;
    if (s.val[PART_XPOS] < this.xmin)
        s.val[PART_XPOS] = this.xmin;
    if (s.val[PART_ZPOS] < this.zmin)
        s.val[PART_ZPOS] = this.zmin;
    if (s.val[PART_XPOS] >= this.xmax)
        s.val[PART_XPOS] = this.xmax;
    if (s.val[PART_YPOS] >= this.ymax)
        s.val[PART_YPOS] = this.ymax;
    if (s.val[PART_ZPOS] >= this.zmax)
        s.val[PART_ZPOS] = this.zmax;
    }
